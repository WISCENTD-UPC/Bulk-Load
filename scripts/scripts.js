...
// key: option ID,
// value: displayName -> code (e.g. string "Yes")
var optionsLegend = new Map();
                ...
                $.getJSON(apiBaseUrl + "/categories/" + val.id + ".json?fields=categoryOptions",
                    function (json) {
                        ...
                        $.each(json.categoryOptions, function (key, val) {
                            queryCategoryOptionCombos(val.id).then(orgUnit => {
                                var sel = document.getElementById('orgList');
                                var opt = document.createElement('option');
                                opt.value = orgUnit;
...
function queryOption(optionId) {
    ...
    $.getJSON(apiBaseUrl + "/options/" + optionId + ".json?&paging=false&" +
        "fields=displayName,code", function (json) {
        //console.log(JSON.stringify(json));
        console.log("key: " + optionId + " name: " + json.displayName + ", code:" + json.code);
        options.set(optionId, json.code);
        optionsLegend.set(optionId, json.displayName + "->" + json.code);
        optionsToQuery--;
    });
}
    ...
    if ((programSelected || forDataSet) && regionalUnitSelected) {
        ...
        if (forDataSet) {
            output_array_sheet_2 = [
                ...
                ["data element ID:", "Section:", "Label:", "Description:", "", "Compulsory?", "Value type:", "Option set Id:", "Possible values (Label -> CODE TO USE):"]
            ];
        } else {
            output_array_sheet_2 = [
        ...
        for (j = 0; j < dataElementsLabel_Array.length; j++) {
            ...
            if (dataElementsHasOptionSet.get(dataElement)) {
                ...
                for (k = 0; k < numOptions; k++) {
                    new_row[8 + k] = optionsLegend.get(ids_of_options[k]);
                }
...
function getImportStrategy() {
    switch (parseInt(getSelectValue("ChooseImportStrategy"))) {
        ...
        default:
            add("Unknown value for import strategy!", 4)
    }
}
    ...
    return new Promise(
        function (resolve, reject) {
            ...
            $.ajax({
                ...
                var ignored = res.response.ignored
                var imported = res.response.imported
                var updated = res.response.updated
...
function showHideDatasets() {
    switch (parseInt(getSelectValue("select_if_data_set_or_program_form"))) {
        ...
        case 2:
            ...
            break;
        default:
            console.log("Invalid option: " + parseInt(getSelectValue("select_if_data_set_or_program_form")))
    }
}
